...
def extract_single_iv(result, prior_sd=0.5):
    ...
    # Convergence diagnostics (additive; does not affect model/prior/sampling)
    diag = az.summary(trace, var_names=['gamma', 'beta'])
    ...
    return {
        ...
        'n_obs': result['n_obs'],
        'gamma_rhat': float(diag.loc['gamma', 'r_hat']),
        'gamma_ess_bulk': float(diag.loc['gamma', 'ess_bulk']),
        'beta_rhat': float(diag.loc['beta', 'r_hat']),
        'beta_ess_bulk': float(diag.loc['beta', 'ess_bulk']),
...
def extract_dual_iv(result, prior_sd=0.5):
    ...
    # Convergence diagnostics (additive; does not affect model/prior/sampling)
    diag = az.summary(trace, var_names=['gamma1', 'gamma2', 'beta'])
    ...
    return {
        ...
        'dv_label': result['dv_label'],
        'n_obs': result['n_obs'],
        'gamma1_rhat': float(diag.loc['gamma1', 'r_hat']),
        'gamma1_ess_bulk': float(diag.loc['gamma1', 'ess_bulk']),
        'gamma2_rhat': float(diag.loc['gamma2', 'r_hat']),
        'gamma2_ess_bulk': float(diag.loc['gamma2', 'ess_bulk']),
        'beta_rhat': float(diag.loc['beta', 'r_hat']),
        'beta_ess_bulk': float(diag.loc['beta', 'ess_bulk']),
...
def generate_report(df, results_4A, results_4B):
    ...
    report.append(f"**生成日期**: {_dt.date.today().isoformat()}")
    ...
    report.append(f"**样本量**: N = {df['user_id'].nunique():,} users × 3 waves (full sample, all 20,576 users)")
    ...
    report.append(f"- Full sample: N = {N_SUBSAMPLE:,} users (all 20,576 users, no subsampling)")
    ...
    for path in [
        os.path.join(REPORT_DIR, "bayesian_riclpm_V105B_fullsample_results.md"),
        os.path.join(OUTPUT_DIR, "bayesian_riclpm_V105B_fullsample_results.md"),
    ]:
    ...
    json_path = os.path.join(OUTPUT_DIR, "bayesian_riclpm_V105B_fullsample_results.json")
    all_results = {
    ...
    return report_text, os.path.join(REPORT_DIR, "bayesian_riclpm_V105B_fullsample_results.md")
...
async def main():
    ...
    try:
        ...
        # Convergence diagnostics summary (R̂ / ESS over all cross-lagged paths)
        rhats, esses = [], []
        for r in results_4A:
            if 'error' not in r and 'gamma_rhat' in r:
                rhats.append(r['gamma_rhat']); esses.append(r['gamma_ess_bulk'])
        for r in results_4B:
            if 'error' not in r and 'gamma1_rhat' in r:
                rhats.extend([r['gamma1_rhat'], r['gamma2_rhat']])
                esses.extend([r['gamma1_ess_bulk'], r['gamma2_ess_bulk']])
        conv_lines = []
        if rhats:
            max_rhat, min_ess = max(rhats), min(esses)
            conv_ok = max_rhat < 1.01 and min_ess >= 400
            conv_lines = [
                "",
                f"收敛诊断: max R̂ = {max_rhat:.4f}, min ESS(bulk) = {min_ess:.0f} "
                f"→ {'全部达标 (R̂<1.01 且 ESS≥400)' if conv_ok else '存在未达标项，需检查'}",
            ]

        abs_report_path = os.path.abspath(report_path)
        summary = "\n".join([
            "Bayesian RI-CLPM分析完成 (修正IV/DV, 方案B, 全样本 N=20,576)",
            "",
            "核心发现:",
            *[f"- {f}" for f in key_findings[:8]],
            *conv_lines,
            "",
            f"完整报告: [bayesian_riclpm_V105B_fullsample_results.md](computer://{abs_report_path})",
        ])
